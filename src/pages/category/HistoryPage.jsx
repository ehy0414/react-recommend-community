...
function HistoryPage() {
    ...
    const getHistoryData = async () => {
      try {
        ...
        const allHistoryData = responses.flatMap((res) =>
          res.data.filter((item) => item.category === "history")
        );
  
        setHistoryPosts(allHistoryData);
      } catch (err) {
    ...
    const sortedHistoryPosts = [...historyPosts].sort((a, b) =>
      sortOrder === "desc" ? b.id - a.id : a.id - b.id
    );
  
    const currentData = sortedHistoryPosts.slice((currentPage - 1) * pageSize, currentPage * pageSize);
    const totalPages = Math.ceil(sortedHistoryPosts.length / pageSize);
  
    return (
      <Wrapper>
        <HeaderTitle title="역사"/>
