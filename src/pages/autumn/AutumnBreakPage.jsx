...
function AutumnBreakPage() {
  const [autumnBreak, setAutumnBreak] = useState([]);
  ...
  const sortedAutumnBreak = [...autumnBreak].sort((a, b) => 
    sortOrder === "desc" ? b.id - a.id : a.id - b.id
  );

  // 현재 페이지에 해당하는 데이터
  const currentData = sortedAutumnBreak.slice((currentPage - 1) * pageSize, currentPage * pageSize);
  const totalPages = Math.ceil(sortedAutumnBreak.length / pageSize);
...
export default AutumnBreakPage;
